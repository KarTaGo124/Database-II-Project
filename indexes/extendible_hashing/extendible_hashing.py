    ...
    def delete(self, secondary_key, bucketfile, index_record_template):
        ...
        while current_bucket is not None:
            for i in range(current_bucket.size):
                ...
                packed_record = bucketfile.read(self.index_record_size)
                if packed_record == tombstone: continue
                try:
...
class ExtendableHashing:
    ...
    def _append_new_bucket(self, local_depth):
        if self.free_pointer == -1:
            ...
            _,_,self.free_pointer = struct.unpack(Bucket.HEADER_FORMAT, self.bucketfile.read(Bucket.HEADER_SIZE))
            self._write_header()
        self.bucketfile.seek(new_pos)
    ...
    def _split_bucket(self, head_bucket: Bucket, new_index_record: IndexRecord):
        ...
        while next_pos != -1:
            ...
            _, _, next_in_chain = struct.unpack(Bucket.HEADER_FORMAT, self.bucketfile.read(Bucket.HEADER_SIZE))
        ...
        for packed_rec in all_records_packed:
            ...
            if isinstance(key, bytes): key = key.decode('utf-8').strip('\x00')
        ...
        while not current_bucket.insert(packed_record, self.bucketfile):
            if current_bucket.next_bucket != -1:
                ...
                self.bucketfile.write(
                    struct.pack(Bucket.HEADER_FORMAT, current_bucket.local_depth, current_bucket.size, new_pos))
                new_bucket = Bucket(new_pos, self.index_record_size, current_bucket.local_depth)
                new_bucket.insert(packed_record, self.bucketfile)
                break
    ...
    def free_bucket(self, bucket_pos):
        self.bucketfile.seek(bucket_pos,0)
        next_free = self.free_pointer
        self.bucketfile.write(struct.pack(Bucket.HEADER_FORMAT, 0, 0, next_free))
